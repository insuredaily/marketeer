/* ==========================================================================
   Interactive Engine for Marketeer
   ========================================================================== */

// Base Simulated Article Database
const localArticles = [
    {
        "title": "Google Core Update Adjusts Search Rank Signals",
        "excerpt": "The latest search engine algorithms prioritize direct user experience metrics and author credentials over generic content.",
        "category": "SEO",
        "author": "Marcus Sterling, Search Lead",
        "date": "2 hours ago",
        "readTime": "4 min read",
        "body": "<p>Google has rolled out a core algorithm update that adjusts how search rankings are determined. The updated system emphasizes direct user experience indicators and verified author credentials.</p><p>Search analysts report shifts in visibility for informational blogs. Websites that rely on generic AI content without unique insights or expert reviews are seeing rank adjustments.</p><p>To adapt, webmasters are advised to focus on building authoritative author bio structures, securing expert reviews for technical claims, and improving mobile layout speeds.</p>",
        "premium": true
    },
    {
        "title": "Navigating the Post-Cookie Era: Zero-Party Data Strategies",
        "excerpt": "With web browsers restricting third-party tracking cookies, marketing teams must build first-party relationships with users.",
        "category": "Ad-Tech",
        "author": "Elena Rostova",
        "date": "4 hours ago",
        "readTime": "5 min read",
        "body": "<p>The restriction of third-party cookies in modern browsers is prompting advertisers to redesign customer tracking strategies.</p><p>To maintain targeting precision, companies are turning to zero-party data\u2014information that customers proactively share through quizzes, interactive surveys, and profile configurations.</p><p>By incentivizing data sharing in exchange for personalized product recommendations, brands are building direct databases that comply with modern privacy standards.</p>",
        "premium": false
    },
    {
        "title": "A/B Testing: How AI-Generated Copy Performs Against Copywriters",
        "excerpt": "We review data comparing AI-generated ad copy and landing page headlines with copy written by experienced copywriters.",
        "category": "Copywriting",
        "author": "David Rostova",
        "date": "1 day ago",
        "readTime": "5 min read",
        "body": "<p>The integration of generative text software in advertising has prompted A/B testing comparing automated copy against human copywriting.</p><p>Data reveals that AI software excels at generating short-form copy variations, such as ad headlines and email subject lines, at speed. However, long-form landing page content written by experienced copywriters consistently reports higher conversion rates.</p><p>The optimal approach combines both methods\u2014using AI to generate options and human copywriters to edit for brand voice and emotional appeal.</p>",
        "premium": true
    }
];
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (isArticlePage) {
        if (artId !== null) {
            ...
            const savedRssArt = localStorage.getItem('10_marketeer_active_rss_article');
            const savedRssList = localStorage.getItem('10_marketeer_rss_articles');
            if (savedRssList) {
...
function loadStateFromStorage() {
    const savedTheme = localStorage.getItem('10_marketeer_dark_mode');
    state.isDarkMode = savedTheme === 'true';

    const savedBookmarks = localStorage.getItem('10_marketeer_bookmarks');
    if (savedBookmarks) {
        state.bookmarks = JSON.parse(savedBookmarks);
    }

    const savedUpvotes = localStorage.getItem('10_marketeer_upvotes');
    if (savedUpvotes) {
        state.upvoted = JSON.parse(savedUpvotes);
    }

    const savedComments = localStorage.getItem('10_marketeer_comments');
    if (savedComments) {
        state.comments = JSON.parse(savedComments);
    }

    const savedSub = localStorage.getItem('10_marketeer_subscribed');
    state.isSubscribed = savedSub === 'true';

    const savedViews = localStorage.getItem('10_marketeer_pageviews');
    if (savedViews) state.pageViews = parseInt(savedViews);

    const savedClicks = localStorage.getItem('10_marketeer_adclicks');
    if (savedClicks) state.adClicks = parseInt(savedClicks);

    const savedEarnings = localStorage.getItem('10_marketeer_earnings');
...
function saveStateToStorage() {
    localStorage.setItem('10_marketeer_dark_mode', state.isDarkMode);
    localStorage.setItem('10_marketeer_bookmarks', JSON.stringify(state.bookmarks));
    localStorage.setItem('10_marketeer_upvotes', JSON.stringify(state.upvoted));
    localStorage.setItem('10_marketeer_comments', JSON.stringify(state.comments));
    localStorage.setItem('10_marketeer_subscribed', state.isSubscribed);
    localStorage.setItem('10_marketeer_pageviews', state.pageViews);
    localStorage.setItem('10_marketeer_adclicks', state.adClicks);
    localStorage.setItem('10_marketeer_earnings', state.earnings.toFixed(2));
}
...
function renderTicker() {
    ...
    const items = ["Google Core Algorithm update adjusts visibility signals for localized business profiles", "Video content loops report 45% higher conversion click-through rates across major channels", "Zero-party data frameworks gain adoption as third-party browser cookies are restricted", "AI copywriting suites demonstrate average conversions increases of 24% in A/B testing", "Retargeting costs increase following updated privacy policies on mobile operating systems"];
...
function handleArticleClick(article) {
    ...
    if (localIdx > -1) {
        ...
        localStorage.setItem('10_marketeer_active_rss_article', JSON.stringify(article));
...
function runWorkspaceCalculator() {
    const resultEl = document.getElementById('calc-results');
            ...
            const target = parseFloat(document.getElementById('revenue-target').value) || 10000;
            const aov = parseFloat(document.getElementById('order-value').value) || 50;
            const conv = parseFloat(document.getElementById('conv-rate').value) / 100 || 0.02;
            const cpc = parseFloat(document.getElementById('avg-cpc').value) || 1.00;
            
            const salesNeeded = target / aov;
            const clicksNeeded = salesNeeded / conv;
            const requiredSpend = clicksNeeded * cpc;
            const roas = (target / requiredSpend) * 100;
            
            resultEl.innerHTML = `
                <div class="calc-result-box" style="border-left: 4px solid var(--primary); padding-left: 10px; margin-top: 15px;">
                    <h5 style="margin: 0 0 5px 0;">Required Budget Projections:</h5>
                    <div style="font-size: 24px; font-weight: bold; color: var(--primary);">$${Math.round(requiredSpend).toLocaleString()}<span style="font-size: 14px; font-weight: normal; color: var(--text-color);"> Est. Ad Spend</span></div>
                    <div style="font-size: 11px; margin-top: 5px; opacity: 0.8;">Clicks Needed: <strong>${Math.round(clicksNeeded).toLocaleString()}</strong> | Conversions: <strong>${Math.round(salesNeeded)}</strong></div>
                    <div style="font-size: 12px; margin-top: 5px; font-weight: bold; color: var(--secondary);">Estimated ROAS: ${roas.toFixed(1)}%</div>
                </div>
            `;
...
<a href="${link}" target="_blank" class="hero-btn" style="display:inline-block; margin-top:10px;">Read Original Source <i class="fa-solid fa-external-link"></i></a>`
            ...
            localStorage.setItem('10_marketeer_rss_articles', JSON.stringify(parsedArticles));
